  ...
  return (
    <>
      ...
      >
        <div className="flex gap-3">
          <div className="avatar relative">
            ...
            {myHighlights.length >= 1 ? (
              <HgCount length={myHighlights.length} />
            ) : (
              <PlusIcon onClick={addHighlight} />
            )}
          </div>
          <div className="">
            ...
            <p className="text-gray-400 font-semibold text-sm">
              {myHighlights.length > 0
                ? `${myHighlights.length} highlights`
                : "Share your moments"}
            </p>
          </div>
        </div>
        ...
        <div
          className="bg-linear-to-r from-purple-500 to-pink-400 hover:from-purple-600 hover:to-pink-700  w-8 h-8 rounded-full flex justify-center items-center hover:brightness-95 transition-all ease-out cursor-pointer"
          onClick={(e) => {
            e.stopPropagation();
            addHighlight();
          }}
